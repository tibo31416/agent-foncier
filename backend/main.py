...
# ══════════════════════════════════════════════════════════════════
# HELPER PDF — streaming + fichier temp (économie mémoire Railway)
# ══════════════════════════════════════════════════════════════════
...
async def _telecharger_pdf(pdf_url: str, zone: str, max_chars: int = 15000) -> dict:
    """
    Télécharge le PDF en streaming vers un fichier temporaire (évite de charger
    tout le PDF en RAM — résout le Out of Memory sur Railway 512 Mo).
    Limite : 25 Mo max.
    """
    ...
    try:
        # ── 1. Téléchargement streaming → fichier temporaire ─────
        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp:
            ...
            async with httpx.AsyncClient(
                ...
                async with client.stream("GET", pdf_url) as r:
                    ...
                    if status != 200:
                        return {"succes": False, "erreur": f"HTTP {status}",
                                "texte": "", "nb_pages": 0, "pages_zone": []}
                    async for chunk in r.aiter_bytes(65536):
                        taille += len(chunk)
                        if taille > MAX_BYTES:
                            return {"succes": False,
                                    "erreur": f"PDF > 25 Mo ({taille//1024//1024} Mo)",
                                    "texte": "", "nb_pages": 0, "pages_zone": []}
                        tmp.write(chunk)

        if taille < 1000:
            return {"succes": False, "erreur": "PDF vide",
                    "texte": "", "nb_pages": 0, "pages_zone": []}

        # ── 2. Extraction texte depuis fichier (hors RAM) ────────
        ...
        for i, page in enumerate(reader.pages):
            ...
            if any([
                ...
                zone_pgs.append((i, t))
            elif i < 20 and any(kw in tu for kw in
                ["DISPOSITION GÉNÉRALE", "DISPOSITION GENERALE", "SOMMAIRE", "DÉFINITION"]):
                gen_pgs.append((i, t))

        zone_idx = {p[0] for p in zone_pgs}
        ctx_pgs  = [
            (i+1, _extraire_page(reader.pages[i+1]))
            for i, _ in zone_pgs
            if i+1 < nb_pages and i+1 not in zone_idx
        ]
        ...
        # Libérer explicitement le reader avant de supprimer le fichier
        del reader
        _gc.collect()
...
# ══════════════════════════════════════════════════════════════════
# ROUTE 4 : ANALYSE CLAUDE — SSE streaming (évite les timeouts)
# Le frontend reçoit des événements au fur et à mesure :
#   event: progress  → message de statut affiché dans l'UI
#   event: result    → JSON final de l'analyse
#   event: error     → message d'erreur
# ══════════════════════════════════════════════════════════════════

def _build_prompt(req: AnalyseRequest, pdf_result: dict) -> str:
    ...
    if pdf_result["succes"]:
        ...
        bloc_pdf = (
            ...
            f"Base-toi sur tes connaissances de la zone {req.zone_plu} (précise que l'analyse est générique)."
        )
    ...
    if bat and bat.get("status") == "ok":
        bloc_batiment = (
            ...
            f"- Niveaux : {bat.get('nb_niveaux')}, Année : {bat.get('annee_construction')}\n"
            f"- Usage : {bat.get('usage')}, DPE : {bat.get('classe_dpe')}"
        )
    else:
        bloc_batiment = "Aucun bâtiment existant (parcelle non bâtie ou données indisponibles)."
    ...
    if tech:
        ...
        if risq.get("zone_inondable"):
            alertes.append("🚨 ZONE INONDABLE — peut bloquer le permis")
        if serv.get("nb_abf", 0) > 0:
            alertes.append(f"⚠ ABF : {', '.join(s.get('nom','') for s in serv.get('abf',[]))} → +3-6 mois PC")
        if serv.get("nb_carrieres", 0) > 0:
            alertes.append("⚠ RISQUE MINIER/CARRIÈRES → étude géotechnique obligatoire")
        if sol.get("nb_cavites", 0) > 0:
            alertes.append(f"⚠ {sol['nb_cavites']} cavité(s) souterraine(s)")
        if sol.get("alea_argiles") and sol["alea_argiles"].upper() in ("FORT", "TRÈS FORT", "MOYEN"):
            alertes.append(f"⚠ ARGILES {sol['alea_argiles']} → fondations renforcées")
        bloc_technique = (
            f"ANALYSE TECHNIQUE :\n"
            f"Sol : argiles={sol.get('alea_argiles','N/A')}, radon={sol.get('radon_categorie','N/A')}, "
            f"cavités={sol.get('nb_cavites',0)}, mvt terrain={sol.get('nb_mvt',0)}\n"
            f"Risques : {', '.join(risq.get('liste_naturels',[])) or 'aucun'}, "
            f"sismicité={risq.get('sismicite','N/A')}, inondable={'OUI' if risq.get('zone_inondable') else 'non'}\n"
            f"Servitudes : {serv.get('nb_abf',0)} ABF, {serv.get('nb_carrieres',0)} carrières, "
            f"{serv.get('nb_reseaux',0)} réseaux, {serv.get('nb_prescriptions',0)} prescriptions PLU\n"
            f"Commune : {com.get('nom','')}, {com.get('population','')} hab\n"
            f"Alertes : {' | '.join(alertes) if alertes else 'aucune'}"
        )
    else:
        bloc_technique = "Données techniques non disponibles."

    return f"""Tu es expert en urbanisme français et promotion immobilière, spécialisé dans l'évaluation foncière pour les promoteurs.

PARCELLE : IDU={req.idu}, Commune={req.commune}, Surface={req.surface_parcelle}m², Zone={req.zone_plu}
{bloc_batiment}
{bloc_technique}
...
══ RÈGLE DE SCORING ══
Le score (0-100) mesure l'opportunité de CRÉER DE LA SURFACE DE PLANCHER.
C'est le critère n°1 — tout le reste en découle.

Pondération :
  40 pts — Surface de plancher créable
            • SDP > 500m²   → 35-40 pts  (excellent)
            • SDP 200-500m² → 20-35 pts  (bien)
            • SDP < 200m²   → 5-20 pts   (limité)
            • Inconstructible → 0 pt

  25 pts — Qualité du droit à construire (PLU)
            • Zone U dense (UG, UA, UB) ou AU → 20-25 pts
            • Zone U résidentielle (UD, UE) → 12-20 pts
            • Zone A, N → 0-5 pts

  20 pts — Risques et contraintes (avec pondération surmontabilité)
            • DÉDUIRE seulement si le risque est BLOQUANT ou très coûteux :
              - Zone rouge PPRI → -15 pts (souvent bloquant)
              - ABF seul        → -3 pts  (surmontable, délai uniquement)
              - ABF + contrainte esthétique forte → -6 pts
              - Carrières PM1   → -5 pts  (surmontable avec étude géotech)
              - Argiles Moyen   → -2 pts  (fondations adaptées suffisent)
              - Argiles Fort    → -5 pts  (surcoût significatif)
              - Cavités         → -8 pts  (risque plus sérieux)
              - ICPE Seveso proche → -5 pts
              - ICPE standard   → 0 pt    (pas d'impact direct)
            • IMPORTANT : un risque surmontable par solution technique ne pénalise pas autant qu'un risque bloquant

  15 pts — Contexte marché et accessibilité
            • Commune dense (>3000 hab/km²) → +10 pts
            • Commune moyenne → +5 pts
            • Voie d'accès correcte → +5 pts
            • Impasse ou voie étroite → 0 pt

IMPORTANT : Cite les articles du règlement PLU quand disponibles.
...
{{
  ...
  "regles_hauteur": {{
    ...
    "regles_specifiques": "description précise avec numéro d'article"
  }},
  "regles_implantation": {{
  ...
  "surface_constructible": {{
    "shon_estimee": "estimation chiffrée en m² pour cette parcelle de {req.surface_parcelle}m²",
    "nb_niveaux_max": "nombre ou null",
    "methode_calcul": "calcul détaillé : surface × emprise × niveaux ou COS"
  }},
  "stationnement": "règles applicables",
  "performances_energetiques": "RE2020 ou autre",
  "contraintes_techniques": {{
    "fondations": "type recommandé selon sol (semelles filantes / radier / pieux)",
    "surmontabilite": "évaluation : les contraintes sont-elles surmontables par des solutions techniques ?",
    "delai_pc_estime": "X mois — justification (ABF +3-6 mois, standard 3-5 mois)",
    "surcoût_estime": "estimation en € ou % du coût construction",
    "points_bloquants": ["uniquement les contraintes vraiment bloquantes, pas juste coûteuses"]
  }},
  "scoring_detail": {{
    "sdp_pts": 0,
    "plu_pts": 0,
    ...
    "marche_pts": 0,
    "total": 0,
    "justification": "explication du score ligne par ligne"
  }},
  "points_attention": ["point 1", "point 2", "point 3"],
  "opportunite_promoteur": {{
    "score": 65,
    "commentaire": "synthèse orientée promoteur : SDP créable, contraintes surmontables ou non, verdict"
  }},
...
async def analyse_plu(req: AnalyseRequest):
    """
    Route SSE (Server-Sent Events) : envoie des événements au fur et à mesure.
    Le navigateur reçoit des données en continu → pas de timeout possible.
    """
    ...
    async def generate():
        ...
        try:
            ...
            yield sse("progress", {"message": "Téléchargement du règlement PLU…", "step": 1})
            pdf_result = await _telecharger_pdf(req.pdf_url, req.zone_plu)

            if pdf_result["succes"]:
                yield sse("progress", {
                    "message": f"PDF lu — {pdf_result['nb_pages']} pages, {len(pdf_result['pages_zone'])} page(s) zone extraites",
                    "step": 2
                })
            else:
                yield sse("progress", {"message": f"PDF inaccessible — analyse générique zone {req.zone_plu}", "step": 2})

            # Étape 2 : construire le prompt
            yield sse("progress", {"message": "Préparation du contexte pour Claude…", "step": 3})
            prompt = _build_prompt(req, pdf_result)

            # Étape 3 : appel Claude
            yield sse("progress", {"message": "Claude analyse le règlement PLU…", "step": 4})
            ...
            yield sse("error", {"message": f"Erreur parsing JSON Claude : {e}"})
        except Exception as e:
            yield sse("error", {"message": f"Erreur : {e}"})
